    ...
    public static Optional<OptionalInt> getCountOfEmployees(HashMap<String, String> employeeTasks,
        ...
        for (Map.Entry<String, String> entry : taskStatuses.entrySet()) {
            ...
            if (nonNullTaskStatus.equalsIgnoreCase("active")) {
                activeTasksList.add(new Task(nonNullTask_id, employeeTasks.get(nonNullTask_id), State.ACTIVE)); // в целом, наличе State.ACTIVE опускает проверку на null этого пол€
            }
        }
...
/*
  Ќаписать метод, принимающий аргументом список, содержащий в себе структуры HashMap<String,String>, описывающие
  объекты типа "«адача проекта".
   ажда€ HashMap<String,String> содержит в себе:
  - "task_id" - строка с идентификатором задач: уникальное значение, повтор€ющихс€ задач в списке нет. Ќе может быть пустой;
  - "assignee_id" - строка с идентификатором назначенного на задачу сотрудника: один сотрудник может быть назначен на несколько
  задач из списка. ћожет быть пустой строкой;
  - "task_state" - строка с названием статуса задачи: принимает одно из двух значений (active|disabled). Ќе может быть пустой;

  ћетод должен вернуть целочисленное значение - количество уникальных сотрудников, назначенных на активные задачи из списка.

  ѕример:
  —писок, переданный как аргумент:
